...
class BinaryCommand(BaseCommand, ABC):
    ...
    def __get_header(self, details) -> List[int]:
    ...
    def __get_footer(self, details) -> List[int]:
    ...
    def _to_long_little_endian(self, word) -> List[int]:
        return [word & 255, (word >> 8) & 255, (word >> 16) & 255, (word >> 24) & 255]

    def _get_crc_bytes(self, data) -> List[int]:
        val = ~(sum(data) % 0x10000) + 1
        val = val & 0xffff  # convert to unsigned value
        return self._to_little_endian(val)

    @abstractmethod
    def _get_command_args(self) -> List[int]:  # pragma: nocover
        pass

    @abstractmethod
    def _get_command_details(self) -> List[int]:  # pragma: nocover
        pass
    ...
    def __validate_canbus_id(self, buffer: bytearray) -> None:
    ...
    def __validate_header_crc(self, buffer: bytearray) -> None:
    ...
    def __validate_details_crc(self, buffer: bytearray) -> None:
